...
export const getWinner = async () => {
  ...
  if (countdown < -15) {
    ...
    const random = Number(randomBig._hex);
    console.log("random", random);
    return [previousWinner, random];
  } else {
    const rounded = Math.round(countdown + 15);
    alert(`The lottery is still open! Still ${rounded} seconds left.`);
  }
};
